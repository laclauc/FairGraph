...
from snapfacebook.src.util.link_prediction import *
...
from snapfacebook import facebook
...
##### for fairwalk
dict_flip = {v: k for k, v in list_node_g_dic.items()}
#####
...
##### for fairwalk
protS_arr = list(protS.values())
prot_arr = [float(i) for i in protS_arr]

int_node_list = [(tups.index(i), i[1]) for i in tups]
dict_protS = dict(int_node_list)
#####
...
for i in range(trials):
    ...
    #path = '/home/manvi/Manvi/Evaluations/snapfacebook/fairwalk1/'
    edge_file = 'fb_fairwalk_graph_train_.edgelist'
    ppp = graph_train.to_networkx()
    nx.write_edgelist(ppp,edge_file,data=False)
    dict_file = 'dict_attr.npy'
    np.save(dict_file, dict_protS)
    out_file = 'emb_fb_fairwalk_graph_train_.emb'
    ...
    def get_embedding(u):
        return np.array(vec_train[dict_flip[u]])
    ...
    vec_train  = fairwalk(input_edgelist=edge_file,output_emb_file=out_file,dict_file=dict_file)
    s_train = protS_arr
