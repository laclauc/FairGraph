...
new_h = nx.relabel_nodes(new_graph, lab)

# Using Stellar library
stellar_polblogs_lap = StellarGraph.from_networkx(new_h)
stellar_polblogs = StellarGraph.from_networkx(h)

print(nx.density(new_h))
print(nx.density(h))
...
for i in range(trials):
    ...
    for k, i in enumerate(examples_test):
        tup = (i[0], i[1])
        labels_test[k] = int(h.has_edge(*tup))

    for k, i in enumerate(examples_train):
        tup = (i[0], i[1])
        labels_train[k] = int(h.has_edge(*tup))

    for k, i in enumerate(examples_model_selection):
        tup = (i[0], i[1])
        labels_model_selection[k] = int(h.has_edge(*tup))
...
with open('results/fb_node2vec_laplace_5.pkl', 'wb') as outfile:
