...
##### for fairwalk
flip_lab_node_list = [(m, n) for n, m in lab_node_list]
dict_flip = dict(flip_lab_node_list)
#####
...
for i in range(trials):
    ...
    gr_nx = graph_train.to_networkx()
    adja = nx.adjacency_matrix(gr_nx) # getting int nodes for fairwalk
    edges = []
    for row in range(adja.shape[0]):
        for col in range(row + 1, adja.shape[0]):
            if adja[row,col]==1:
                edges.append((row, col))

    #for fairwalk write edgelist and write protS in dict form
    edge_file = 'polblog_fairwalk_00.edgelist'
    f = open(edge_file, 'w+')
    for i in edges:
        f.write(str(i[0])+" "+str(i[1])+'\n')
    f.close()
    dict_file = 'polblogs_dict_attr00.npy'
    np.save(dict_file, protS_int)
    out_file = 'polblogs_fairwalk_check00.emb'
    #print(protS)
    ...
    print("Start fairwalk on the graph train")
